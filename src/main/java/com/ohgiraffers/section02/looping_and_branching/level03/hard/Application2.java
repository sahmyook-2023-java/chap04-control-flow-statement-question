...
public class Application2 {
	...
	public static void main(String[] args) {
		...
		int count = 0;							//쵯수를 누적할 변수 초기화
		int random = (int)(Math.random()*100)+1;			//난수 초기화

		System.out.print("정수를 입력하세요 : ");
		int getNum = sc.nextInt();					//사용자의 입력을 받음
		count++;									//횟수누적
		...
		while(true) {
			if (random > getNum) {
				...
				System.out.print("정수를 입력하세요 : ");
				getNum = sc.nextInt();
			} else if (random < getNum) {
				System.out.println("입력하신 정수보다 작습니다.");
				count++;
				System.out.print("정수를 입력하세요 : ");
				getNum = sc.nextInt();
			} else {
				System.out.println("정답입니다. " + count + "회만에 정답을 맞추셨습니다."); break;
			}
